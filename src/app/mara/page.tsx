...
import { getLatestSnapshot, getMarginBuckets, getSkuCount, getSlotCount, type SlotAnalytics } from '@/lib/vendetti/mara/analytics';
import { avatarUrl, TEAM } from '@/lib/agents/team';
...
export default async function MaraPage() {
  const [snap, buckets, slotCount, skuCount] = await Promise.all([
    getLatestSnapshot(),
    getMarginBuckets(),
    getSlotCount(),
    getSkuCount(),
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      <header className="mb-8 flex items-start gap-5">
        ...
        />
        <div>
          <h1 className="text-3xl font-bold text-navy">Mara · Análise</h1>
          <p className="mt-1 text-navy/60 italic">"{mara.tagline}"</p>
          <p className="mt-2 text-sm text-navy/55">
            Último sync: <strong>{lastSync}</strong> · Rode <code className="rounded bg-navy-50 px-1.5 py-0.5">npm run mara:sync</code> pra atualizar.
          </p>
        </div>
      </header>
      ...
      <section className="grid gap-3 sm:grid-cols-4">
        ...
        <Kpi label="Capacidade preenchida" value={`${capacity.toFixed(1)}%`} tone={capacity < 40 ? 'red' : capacity < 70 ? 'amber' : 'emerald'} />
        <Kpi label="Snapshots no histórico" value="1" />
      </section>
      ...
      {buckets.low.length > 0 && (
        <section className="mt-8 rounded-lg border border-rose-200 bg-rose-50 p-5">
          ...
          <p className="mt-1 text-sm text-rose-800">
            Vendetti deve revisar preço, custo cadastrado, ou estratégia de phase-out. Mentos Kiss já foi marcado como phase-out
            intencional pelo Luís — os outros precisam decisão.
          </p>
        </section>
      )}

      <footer className="mt-10 text-xs text-navy/50">
        Mara consulta o Postgres direto — todas essas análises rodam em milissegundos. O sync (extract + load) leva ~12-15s e
        deve rodar via cron diário. Ver <Link href="/equipe" className="underline">/equipe</Link> pra a hierarquia completa.
      </footer>
    </main>
  );
}
